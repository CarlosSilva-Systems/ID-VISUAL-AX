...
@router.post("/{mac_address}/bind")
async def bind_device(
    mac_address: str,
    req: BindRequest,
    session: AsyncSession = Depends(get_session),
):
    """Vincula device por MAC address (legado)."""
    ...
    return {"id": str(device.id), "mac_address": device.mac_address, "workcenter_id": device.workcenter_id}


@router.delete("/{mac_address}/bind")
async def unbind_device(
    mac_address: str,
    session: AsyncSession = Depends(get_session),
):
    """Desvincula device por MAC address (legado)."""
    stmt = select(ESPDevice).where(ESPDevice.mac_address == mac_address)
    result = await session.execute(stmt)
    device = result.scalars().first()
    if not device:
    ...
    return {"id": str(device.id), "mac_address": device.mac_address, "workcenter_id": None}
