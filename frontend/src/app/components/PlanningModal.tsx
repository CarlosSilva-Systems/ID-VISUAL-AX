        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                <div className="p-8 border-b border-slate-50 flex items-center justify-between">
                    <div>
                        <h2 className="text-xl font-black text-slate-900 tracking-tight">Planejamento</h2>
                ...
                <div className="p-8 max-h-[60vh] overflow-y-auto">
                    {plannedMos.length === 0 ? (
                        <div className="text-center py-12 space-y-3">
                            ...
                            <p className="text-slate-400 font-bold">Sem ordens planejadas no momento.</p>
                        </div>
                    ) : (
                                ...
                                >
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2 text-blue-600">
                                            <Package size={16} />
                                            <span className="font-black text-sm">{mo.mo_name}</span>
                                        </div>
                                    ...
                                    <div className="flex items-center justify-between text-xs text-slate-500">
                                        <div className="flex items-center gap-1.5">
                                            <Clock size={14} />
                                            <span>{mo.date_start ? new Date(mo.date_start).toLocaleString('pt-BR') : 'Sem data definida'}</span>
                                        </div>
                                    </div>
                                </div>
                            ))}
